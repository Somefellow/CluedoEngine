...
class DeductionRandomBot < Bot
  ...
  def do_not_have_these(player_id, cards)
    cards.each do |card|
      @players_possible_cards.each do |player_possible_cards|
        player_possible_cards.delete(card)
        @possible_cards.delete(player_possible_cards[0]) if player_possible_cards.count == 1
      end
    end
  end

  def do_have_one_of_these(player_id, cards)
    player_possible_cards = cards.select { |card| @possible_cards.include?(card) }
    if player_possible_cards.count == 1
      @possible_cards.delete(player_possible_cards[0])
    elsif player_possible_cards.count > 1
      @players_possible_cards[player_id].push(player_possible_cards)
    end
  end
